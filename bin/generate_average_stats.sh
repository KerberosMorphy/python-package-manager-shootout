...
RUN_COUNT=4
RUN_LIST=$(gh run list --workflow benchmark --json databaseId,event,conclusion --limit 50 --jq 'map(select(.event=="workflow_dispatch" and .conclusion=="success")) | .[].databaseId')
